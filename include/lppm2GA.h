/* $Id: lppm2GA.h,v 1.2 2009-01-07 12:46:33 mcouprie Exp $ */
#ifdef __cplusplus
extern "C" {
#endif
extern int32_t lpgm2ga(struct xvimage *im, struct xvimage *ga, int32_t param, double alpha);
extern int32_t lpgm2gafloat(struct xvimage *im, struct xvimage *ga, int32_t param, double alpha);
extern int32_t lpgm2gaDouble(struct xvimage *im, struct xvimage *ga, int32_t param, double alpha);
extern int32_t lpgm2gaDouble(struct xvimage *im, struct xvimage *ga, int32_t param, double alpha);
extern int32_t lppm2ga(struct xvimage *r, struct xvimage *v, struct xvimage *b, struct xvimage *ga, int param);
extern int32_t lpgm2ga4d(struct xvimage4D *im, struct GA4d * ga, int param);
#ifdef __cplusplus
}
#endif
