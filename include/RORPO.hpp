...
T* Call_RORPO(T* image_modif, std::vector<int>S_list, int nb_scales, int nb_core, int max_value, int dimx, int dimy, int dimz, int debug_flag, char* mask_buffer=NULL)
{
	...
	for (it=S_list.begin();it!=S_list.end();++it)
	{
		...
		// ----------------- Max of scales ---------------
	    multiscale=max_crush<T>(multiscale,output_buffer,dimx,dimy,dimz);
	}
	...
	multiscale=min_crush<T>(multiscale,input_buffer,dimx,dimy,dimz);
...
T* RORPO(T* input_buffer, T* image_dilat, int L, int nb_core, int dimx, int dimy, int dimz)
{
   ...
   // Buffer for results
    ...
    // ######################################### Limit Orientations Treatment #########################################
     ...
     Imin4_1=min_crush(Imin4_1,res3,dimx,dimy,dimz);
     Imin4_1=min_crush(Imin4_1,res6,dimx,dimy,dimz);

     //horizontal + vertical + diag2 + diag3
     T* Imin4_2=min_alloc(res0,res1,dimx,dimy,dimz);
     Imin4_2=min_crush(Imin4_2,res4,dimx,dimy,dimz);
     Imin4_2=min_crush(Imin4_2,res5,dimx,dimy,dimz);

     //horizontal + profondeur + diag2+ diag4
     T* Imin4_3=min_alloc(res0,res2,dimx,dimy,dimz);
     Imin4_3=min_crush(Imin4_3,res4,dimx,dimy,dimz);
     Imin4_3=min_crush(Imin4_3,res6,dimx,dimy,dimz);

     //horizontal + profondeur + diag1+ diag3
	 T* Imin4_4=min_alloc(res0,res2,dimx,dimy,dimz);
     Imin4_4=min_crush(Imin4_4,res3,dimx,dimy,dimz);
     Imin4_4=min_crush(Imin4_4,res5,dimx,dimy,dimz);

     //vertical + profondeur + diag1+ diag2
     T* Imin4_5=min_alloc(res1,res2,dimx,dimy,dimz);
     Imin4_5=min_crush(Imin4_5,res3,dimx,dimy,dimz);
     Imin4_5=min_crush(Imin4_5,res4,dimx,dimy,dimz);

     //vertical + profondeur + diag3+ diag4
     T* Imin4_6=min_alloc(res1,res2,dimx,dimy,dimz);
     Imin4_6=min_crush(Imin4_6,res5,dimx,dimy,dimz);
     Imin4_6=min_crush(Imin4_6,res6,dimx,dimy,dimz);

     // Final
     T* Imin4=max_alloc(Imin4_1,Imin4_2,dimx,dimy,dimz);
     Imin4=max_crush(Imin4,Imin4_3,dimx,dimy,dimz);
     Imin4=max_crush(Imin4,Imin4_4,dimx,dimy,dimz);
     Imin4=max_crush(Imin4,Imin4_5,dimx,dimy,dimz);
     Imin4=max_crush(Imin4,Imin4_6,dimx,dimy,dimz);

	 // ---- Imin limit case 5 orientations ----
    T* Imin5=min_alloc(res3,res4,dimx,dimy,dimz);
    Imin5=min_crush(Imin5,res5,dimx,dimy,dimz);
    Imin5=min_crush(Imin5,res6,dimx,dimy,dimz);
    ...
    RPO_RPO_limit_orientations=max_crush(RPO_RPO_limit_orientations,diff_Imin5,dimx,dimy,dimz);
...
// ####################  Run PO for each orientation ##############################
	...
	min_crush<T>(res1, input_buffer, dimx, dimy, dimz);
	min_crush<T>(res2, input_buffer, dimx, dimy, dimz);
	min_crush<T>(res3, input_buffer, dimx, dimy, dimz);
	min_crush<T>(res4, input_buffer, dimx, dimy, dimz);
	min_crush<T>(res5, input_buffer, dimx, dimy, dimz);
	min_crush<T>(res6, input_buffer, dimx, dimy, dimz);
	min_crush<T>(res7, input_buffer, dimx, dimy, dimz);
...
template<typename T>
T* max_crush(T* I1, T* I2, int dimx, int dimy, int dimz)
...
//The result is stored in I1
template<typename T>
T* min_crush(T* I1, T* I2, int dimx, int dimy, int dimz)
{
    struct xvimage * image1;
    struct xvimage * image2;
    int32_t typepixel;
...
template<typename PixType>
static void RORPO_multiscale(PixType* image, PixType* output, int Smin, int factor, int nb_scales, int nb_core, int dimx, int dimy, int dimz, PixType* mask_buffer==NULL, int debug_flag==-1)
{
...
// ################################# Scales Computations ###################################
	...
	// Run RORPO
	if (mask_buffer!=NULL){
		output=Call_RORPO<PixType>(image, S_list, nb_scales,nb_core, max_value, dimx, dimy, dimz, debug_flag, mask_buffer);	
		delete[](mask_buffer);
	}
	else 
		output=Call_RORPO<PixType>(image, S_list, nb_scales,nb_core, max_value, dimx, dimy, dimz, debug_flag);	
