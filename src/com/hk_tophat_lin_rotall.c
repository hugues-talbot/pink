...
int argc; char **argv; 
{
    struct xvimage * image; // Pointer fŸr das Bild
    int32_t rs;    // row_size ^= width, erhŠlt man mit(s.u.): rowsize(image)
    int32_t cs;    // col_size ^= height, erhŠlt man mit(s.u.): colsize(image)
    int32_t d;     // dimesion, erhŠlt man mit(s.u.): depth(image)
