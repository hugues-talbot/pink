...
int32_t IsMA(int32_t x, int32_t y, 
	     Image image, int32_t rs, int32_t cs,
...
int32_t IsMA3d(int32_t x, int32_t y, int32_t z,
	       Image image, int32_t rs, int32_t cs, int32_t ds,
	       MLut mlut, RTLutCol Lut)
//----------------------------------
{
...
/*     rmax= RadiusMax(imagedist, rs, cs, ds); */
/* #ifdef VERBOSE */
/*     printf("Maximum radius in the image=%d\n",rmax); */
/* #endif */
/*     if (rmax > rknown) */
...
/* 	if(imagedist[i*rs + j]!=0) */
/* 	{ */
/* 	  distance=imagedist[i*rs + j]; */
/* 	  if (CallMedial(i, j, imagedist, rs, cs, MgL1, LutColumn1, rknown, rmax))	  */
...
/* 	  if (imagedist[i*ps + j*rs + k] != 0) */
/* 	  { */
/* 	    distance=imagedist[i*ps + j*rs + k]; */
/* 	    if (CallMedial3d(i, j, k, imagedist, rs, cs, ds, MgL1, LutColumn1, rknown, rmax))	  */
