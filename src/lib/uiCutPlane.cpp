...
namespace pink {
  ...
  MatrixXd ortho_normalize( MatrixXd simple )
  {
    ...
    orient << simple(0,0), simple(0,1), simple(0,2);
    //DVECT(orient);
    
    // reference is autogenerated
    axe << simple(2,0), simple(2,1), simple(2,2);
    //DVECT(axe);
    
    
    v3d new_reference = v3d(axe.cross(orient));
    v3d new_orient = v3d(new_reference.cross(axe));
  ...
  MatrixXd transformation( MatrixXd image, MatrixXd projection )
  {
    
    MatrixXd result;
    result = image * projection.inverse();

    return result;
  } /* transformation */
  

  MatrixXd rotate_axe( Vector3d axe, Vector3d origin, double angle )
  {
...
//     new_base.row(2).swap(new_base.row(1));
