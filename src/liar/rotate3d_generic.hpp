   ...
   double gamma)	/* shear factor in radians */
{
    ...
    res = XCAT2(trimborder_3d,F_TYPE)(in, out, PIX_CODE, borderx, bordery, borderz,\
...

{
	...
	*q++ = liar_rint((1-area4)*fillval + (*p)*area4);	/* first point */
	for (x = 1 ; x < nxin ; x++, p++)
	    *q++ = liar_rint((1-area3-area4)*fillval + (*p)*area3 + *(p+1)*area4);
	if (area3 > 0.0)
	    *q = liar_rint((1-area3)*fillval + (*p)*area3); /* last point */
	...
	for (y = 1 ; y < nyin ; y++) {
	    ...
	    if (startx == startx1) {
		count = nxin - 1;
		*q++ = liar_rint((area1+area3)*fillval + (*p)*area4 + (*r)*area2);
	    } else if ((startx1 - startx) == 1) {
		count = nxin - 2;
		*q++ = liar_rint((1-area4)*fillval + (*p)*area4);
		*q++ = liar_rint(area1*fillval + (*p)*area3 + *(p+1)*area4 + (*r)*area2);
		p++;
	    } else {
		count = nxin - 2;
		*(q-1) = liar_rint((1-area2)*fillval + (*r)*area2);
		*q++ = liar_rint(area3*fillval + (*p)*area4 + (*r)*area1 + *(r+1)*area2);
		r++;
	    }

	    for (x = 1 ; x <= count ; x++, p++, r++)
		*q++ = liar_rint((*p)*area3 + *(p+1)*area4 + (*r)*area1 + *(r+1)*area2);
	    if (area3 > 0.0 || area1 > 0.0) {
	    	if (startx == startx1)
		    *q = liar_rint((area2+area4)*fillval + (*p)*area3 + (*r)*area1);
		else if ((startx1 - startx) == 1) {
		    if (nxin > 1)
			*q++ = liar_rint(area4*fillval + (*p)*area3 + (*r)*area1 + *(r+1)*area2);
		    *q = liar_rint((1-area1)*fillval + *(r+1)*area1);
		}
		else {
		    if (nxin > 1)
			*q++ = liar_rint(area2*fillval + (*p)*area3 + *(p+1)*area4 + (*r)*area1);
		    *q = liar_rint((1-area3)*fillval + *(p+1)*area3);
		}
	    }
	}
	...
	if (err3 > 0.0) { /* extra row required */
	    *q++ = liar_rint((1-area2)*fillval + (*p)*area2);
	    for (x = 1 ; x < nxin ; x++, p++)
		*q++ = liar_rint((1-area1-area2)*fillval + (*p)*area1 + *(p+1)*area2);
	    if (area1 > 0.0)
		*q = liar_rint((1-area1)*fillval + (*p)*area1);
	}
    }
    return 0;
}
...

{
	...
	*q = liar_rint((1-area4)*fillval + (*p)*area4);	/* first point */
	q += (*nxout) * (*nyout);
	for (z = 1 ; z < nzin ; z++) {
	    *q = liar_rint((1-area3-area4)*fillval + (*p)*area3 + *(p+nxin*nyin)*area4);
	    q += (*nxout) * (*nyout);
	    p += nxin * nyin;
	}
	if (area3 > 0.0)
	...
	for (y = 1 ; y < nyin ; y++) {
		...
		*q = liar_rint((1-area4)*fillval + (*p)*area4);
		q += (*nxout) * (*nyout);
		*q = liar_rint(area1*fillval + (*p)*area3 + *(p+nxin*nyin)*area4 + (*r)*area2);
		q += (*nxout) * (*nyout);
		p += nxin * nyin;
	    } else {
		count = nzin - 2;
		*(q-(*nxout)*(*nyout)) = liar_rint((1-area2)*fillval + (*r)*area2);
		*q = liar_rint(area3*fillval + (*p)*area4 + (*r)*area1 + *(r+nxin*nyin)*area2);
		q += (*nxout) * (*nyout);
		r += nxin * nyin;
	    }

	    for (z = 1 ; z <= count ; z++) {
		*q = liar_rint((*p)*area3 + *(p+nxin*nyin)*area4 + (*r)*area1 + *(r+nxin*nyin)*area2);
		q += (*nxout) * (*nyout);
		p += nxin * nyin;
		r += nxin * nyin;
	    }
	    if (area3 > 0.0 || area1 > 0.0) {
		    ...
		    *q = liar_rint((area2+area4)*fillval + (*p)*area3 + (*r)*area1);
		    ...
		    if (nzin > 1) {
			*q = liar_rint(area4*fillval + (*p)*area3 + (*r)*area1 + *(r+nxin*nyin)*area2);
			q += (*nxout) * (*nyout);
		    }
		    *q = liar_rint((1-area1)*fillval + *(r+nxin*nyin)*area1);
		}
		else {
		    if (nzin > 1) {
			*q = liar_rint(area2*fillval + (*p)*area3 + *(p+nxin*nyin)*area4 + (*r)*area1);
			q += (*nxout) * (*nyout);
		    }
		    *q = liar_rint((1-area3)*fillval + *(p+nxin*nyin)*area3);
		}
	    }
	}
	...
	if (err3 > 0) {	/* more rows are required */
	    ...
	    *q = liar_rint((1-area2)*fillval + (*p)*area2);
	    q += (*nxout) * (*nyout);
	    for (z = 1 ; z < nzin ; z++) {
		*q = liar_rint((1-area1-area2)*fillval + (*p)*area1 + *(p+nxin*nyin)*area2);
		q += (*nxout) * (*nyout);
		p += nxin * nyin;
	    }
	    if (area1 > 0.0)
   ...
   int rotz)		/* rotational angle about Z */
{
    ...
    if ((tmpbuf =  malloc(nx * ny * nz * sizeof(Type))) == NULL) {
	LIARerror("Orthorot_3d: insufficient memory");
	return(1);
    }
    if (x == 180 && y == 180 && z == 180) {
	if ((res = XCAT2(pointsymm_3d,F_TYPE)(bufin, tmpbuf, nx, ny, nz)) != 0)
	    return(1);
    } else if (x == 90 && y == 90 && z == 90) {
	if ((res = XCAT2(yrotp90_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	...
	if (x == 90) {
	    ...
	    if ((res = XCAT2(xrotp90_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	    ...
	    /* do a rotation of -90 degrees... */
	    if ((res = XCAT2(xrotm90_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	    ...
	    if ((res = XCAT2(xrot180_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	...
	if (y == 90) {
	    ...
	    /* do a rotation of -90 degrees... */
	    if ((res = XCAT2(yrotm90_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	    ...
	    /* do a rotation of +180 degrees... */
	    if ((res = XCAT2(yrot180_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	...
	if (z == 90) {
	    /* do a rotation of 90 degrees... */
	    if ((res = XCAT2(zrotp90_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	    ...
	    /* do a rotation of -90 degrees... */
	    if ((res = XCAT2(zrotm90_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
	    ...
	    /* do a rotation of +180 degrees... */
	    if ((res = XCAT2(zrot180_3d,F_TYPE)(tmpbuf, tmpbuf, nx, ny, nz)) != 0)
   ...
   int slice0)               /* col center of rot */
{
	...
	if (alpha > 135.0) {
	    XCAT2(zrot180_3d,F_TYPE)(tmpinbuf, tmpinbuf, inx, iny, inz);
	    alpha -= 180.0;
	} else {
	    /* we need another intermediary image */
	    XCAT2(zrotp90_3d, F_TYPE)(tmpinbuf, tmpinbuf, inx, iny, inz);
	    alpha -= 90.0;
	...
	if (alpha < -135.0) {
	    ...
	    XCAT2(zrotm90_3d, F_TYPE)(tmpinbuf, tmpinbuf, inx, iny, inz);
	    alpha += 90.0;
	    SWAPINT(inx, iny);
	}
    }
	...
	*fnz = nz;
	...
	if (interpolate == 1)
	    res = XCAT2(shear1_itp, F_TYPE)(bufin, bufout, nx, ny, nz, &onx, &ony, &onz, \
			     a, b, c, value);
	else
	    res = XCAT2(shear1_nn, F_TYPE)(bufin, bufout, nx, ny, nz, &onx, &ony, &onz, \
			    a, b, c, value);
	if (res == 0) {
	    ...
	    if (interpolate == 1)
		res = XCAT2(shear2_itp, F_TYPE)(*bufout, &tmpbuf, onx, ony, onz, &nx, &ny, \
				 &nz, a, b, c, value);
	    else
		res = XCAT2(shear2_nn, F_TYPE)(*bufout, &tmpbuf, onx, ony, onz, &nx, &ny, \
				&nz, a, b, c, value);
	    if (res == 0) {
		free(*bufout);
		a = myround(-tan((radian1+radian3)/2));
		...
		if (interpolate == BILINEAR_INTERP)
		    res = XCAT2(shear1_itp, F_TYPE)(tmpbuf, bufout, nx, ny, nz, &onx, &ony, \
				    &onz, a, b, c, value);
		else
		    res = XCAT2(shear1_nn, F_TYPE)(tmpbuf, bufout, nx, ny, nz, &onx, &ony, \
				    &onz, a, b, c, value);
		if (res == 0) {
		    free(tmpbuf);
		    switch (rmbdr) {
			...
			*fnz = onz;
			break;
		      ...
		      case CALC_BORDER:
			res = XCAT2(pullrotated_3d, F_TYPE)(*bufout, &tmpbuf, onx, ony, onz, \
					     inx, iny, inz, fnx, fny, fnz, \
					     alpha, beta, gamma);
			break;

		      case EAT_BORDER:
			res = XCAT2(rmborder_3d, F_TYPE)(*bufout, &tmpbuf, onx, ony, onz, \
					  fnx, fny, fnz, value);
			break;

		      case SAME_SIZE:
			res = XCAT2(pullcenteredsubset_3d, F_TYPE)(*bufout, &tmpbuf, onx, ony, onz,
