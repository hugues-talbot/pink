...
namespace pink {
  namespace python {
      ...
      )
    {
      ...
      {
        pinhibit = inhibit.get_output();        
      } /* NOT inhibit.isnull() */      
      
      if (! lskelsurf3d(result.get_output(), priority_image, pinhibit, connexity))
      {
...
UI_EXPORT_FUNCTION(
  ...
  ( arg("image"), arg("priority_image"), arg("connexity"), arg("inhibit")="NULL" ),
