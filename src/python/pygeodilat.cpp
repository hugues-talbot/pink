...
PTR<char_image> py_geodilat( const char_image & F, 
			     const char_image & G, 
			     int connex, 
			     int numb_iter ){
  ...
  {
    lgeodilat( &(*result), &(*G_const_away), connex, numb_iter ); 
  ...
  { /* NOT result.get_size().size()==2 */
    ...
    {      
      lgeodilat3d( &(*result), &(*G_const_away), connex, numb_iter );
      // &(*result) == result->operator& and it returns an "xvimage*"
      // &(*G_const_away) == G_const_away->operator& and it returns an "xvimage*"
    } 
    else 
