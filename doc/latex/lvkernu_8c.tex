\section{lvkernu.c File Reference}
...
{\bf Usage:} lvkernu in.pgm $<$imcond.pgm$|$null$>$ connex out.pgm

{\bf Description:} Grayscale ultimate leveling thickening (refs. [BEC97, CBB01]). The parameter {\bf connex} gives the connectivity used for the minima; possible choices are 4, 8 in 2D and 6, 26 in 3D. Let F be the function corresponding to the input image {\bf in.pgm}. Let G be the function corresponding to the input image {\bf imcond.pgm}, or the blanck function (constant 255) if the keyword {\bf null} is used. The algorithm is the following:
